...
def select_and_build_image(machine_request):
    old_provider = machine_request.instance.provider_machine.provider.type.name.lower()
    ...
    if old_provider == 'eucalyptus':
        if new_provider == 'eucalyptus':
            ...
            new_image_id = manager.create_image(
                ...
                public=True if "public" in machine_request.new_machine_visibility.lower() else False,
...
def machineImagingTask(machine_request):
    try:
        ...
        if new_image_id == None:
            raise Exception('The image cannot be built as requested. The provider combination is probably bad.')
        logger.info('New image created - %s' % new_image_id)

        #Build the new provider-machine object and associate
        new_machine = createProviderMachine(machine_request.new_machine_name, new_image_id, machine_request.new_machine_provider_id)
        generic_mach = new_machine.machine
        tags = [Tag.objects.get(name=tag) for tag in machine_request.new_machine_tags.split(',')] if machine_request.new_machine_tags else []
        ...
        machine_request.save()
        send_image_request_email(machine_request.new_machine_owner, machine_request.new_machine, machine_request.new_machine_name)
        return new_image_id
    except Exception, e:
        logger.error(e)
