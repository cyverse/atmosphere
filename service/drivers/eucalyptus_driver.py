...
class Eucalyptus_Esh_NodeDriver(EucNodeDriver):
    ...
    def _drop_numbers(self, machine_name):
        return '_'.join(filter(lambda x: not str.isdigit(x), machine_name.split('_')))
    ...
    def parse_machine_name(self, machine_name):
        new_machine_name = self._drop_numbers(self._drop_filetype(self._drop_dir(machine_name)))
    ...
    def list_images(self, location=None, ex_image_ids=None, emi_only=True):
        ...
        """
        image_list = super(Eucalyptus_Esh_NodeDriver, self).list_images(location,ex_image_ids)
        if emi_only:
            image_list = [img for img in image_list if 'eri' not in img.id and 'eki' not in img.id]
        return image_list

    def _to_image(self, element):
       n = NodeImage(
           id=findtext(element=element, xpath='imageId', namespace=NAMESPACE),
           name=self.parse_machine_name(findtext(element=element, xpath='imageLocation',
                         namespace=NAMESPACE)),
    ...
    def _getNextAvailableDevice(self, instance_id):
        """
        Given an instance_id, return the next available location for attaching a volume
        relative to /dev/
        """
        def _attached_to_instance(vol):
            logger.warn(vol)
            logger.warn(vol.__dict__)
            attached_instance_id = vol.extra.get('attachmentSet',{}).get('instanceId','')
            return attached_instance_id == instance_id
        
        # get all volumes that are attached to this instance, add devices to list
        attached_volumes = filter(_attached_to_instance, self.list_volumes())
        used_devices = [vol.extra.get('attachmentSet',{}).get('device') for vol in attached_volumes]

        logger.warn(used_devices)

        # start with the letter b, increment until "xvd[char]" is not in the list of used devices
        ...
        # warning: this will fail if you try to attach > 25 volumes.  Probably not a typical use case, though
    ...
    def attach_volume(self, node, volume, device=None):
        ...
        return super(Eucalyptus_Esh_NodeDriver, self).attach_volume(node,volume,device)
