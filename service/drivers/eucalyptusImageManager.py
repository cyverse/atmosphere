...
class ImageManager():
    ...
    def create_image(self, instance_id, image_name, public=True,
        ...
        # mount and clean image, upload it, then remove it
        image_path = self._download_remote_image(
            instance_id, image_path, remote_img_path)
        self._clean_local_image(
            image_path, '%s/mount/' % local_download_dir, exclude=exclude)
        new_image_id = self._upload_local_image(
    ...
    def _openstack_kvm_export(self, image_path, download_dir):
        ...
        #REMOVE (1-line):
        for (remove_line_w_str, remove_from) in [("alias scsi",
                                                  "etc/modprobe.conf"),
                                                 ("atmo_boot",
                                                  "etc/rc.local")]:
            replace_file_path = os.path.join(mount_point, remove_from)
            if os.path.exists(replace_file_path):
                self.run_command(["/bin/sed", "-i", "/%s/d"
                                  % remove_line_w_str, replace_file_path])
...
""" % (rmdisk_version, rmdisk_version, root_string, latest_rmdisk)
    ...
    def download_instance(self, download_dir, instance_id,
        ...
        if not local_img_path:
            meta_name = '%s_%s_%s_%s' % ('admin', owner, instance.id,
        ...
        return self._download_remote_image(instance_id,
                                           local_img_path, remote_img_path)

    def run_command(self, commandList, stdout=subprocess.PIPE,
                    stderr=subprocess.PIPE, stdin=None):
        """
        Using Popen, run any command at the system level
        and record the output and error streams
        """
        out = None
        err = None
    ...
    def _download_remote_image(self, instance_id,
                               local_img_path, remote_img_path):
        """
    ...
    def _build_instance_nc_map(self):
        ...
        boto_instances = self.image_conn.get_list(
            "DescribeNodes", {}, [('euca:item', Instance)], '/')
        last_node = ''
        nodes = {}
        instances = {}
