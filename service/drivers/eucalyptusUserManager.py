...
class UserManager():
  """
  Accessing Eucalyptus' Admin panel works just like any other EC2-compliant query
  HOWEVER, the default path for admin is '/services/Accounts/' instead of '/services/Eucalyptus'!
  """
  admin_key     = None 
  admin_secret  = None
  admin_ec2_url = None
  ...
  def __init__(self, key=settings.EUCA_ADMIN_KEY, secret=settings.EUCA_ADMIN_SECRET, url=settings.EUCA_EC2_URL, path='/services/Accounts'):
    ...
    self.connection = boto.connect_ec2(aws_access_key_id=self.admin_key, aws_secret_access_key=self.admin_secret, is_secure=False, region=region, port=parsed.port, path=self.account_path)
  ...
  def delete_user(self, username):
    ...
    """
    try:
      reply = self.connection.get_object('DeleteUser', {'UserName':username}, BooleanResponse)
    except EC2ResponseError, resp_err:
  ...
  def add_user(self, username, email="N/A", adminUser=False):
    ...
    returns True on success
    """
    reply = self.connection.get_object('AddUser', {'UserName':username, 'Email':email, 'Admin':adminUser}, BooleanResponse)
    return reply and reply.reply == 'true'
  ...
  def get_keys(self, userList):
    ...
    for username in userDict.keys():
        ...
        user_keys_dict[username] = {'username':username, 'access_key': userObj['access_key'], 'secret_key' : userObj['secret_key']}
  ...
  def get_users(self, userList):
    ...
    logging.warn('\n\n%s\n\n' % params)
    euca_list = self.connection.get_list('DescribeUsers', params, [('euca:item', User)])
    return self.userListDict(euca_list)

  def userListDict(self, euca_list):
    """
    Utility method to turn a user_list (XML Parsed object from euca_admin) into python dict
    """
    userDict = {}
