...
def filter_events_and_instances(report_start_date, report_end_date, username=None):
    ...
    if username:
        ...
        events = events.filter(Q(entity_id=username)).order_by('timestamp')
...
def get_allocation_source_name_from_event(username, report_start_date, instance_id):
    events = EventTable.objects.filter(Q(timestamp__lt=report_start_date) & Q(name__exact="instance_allocation_source_changed") & Q(entity_id=username) & Q(payload__instance_id__exact=instance_id)).order_by('timestamp')
    if not events:
        return False
    else:
