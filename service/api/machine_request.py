...
class MachineRequestList(APIView):
    ...
    def get(self, request, provider_id, identity_id):
        ...
        all_user_reqs = CoreMachineRequest.objects.filter(new_machine_owner=request.user)
    ...
    def post(self, request, provider_id, identity_id):
        ...
        if serializer.is_valid():
            ...
            approve_link = '%s/api/request_image/%s/approve' % (settings.SERVER_URL , machine_request_id)
            deny_link = '%s/api/request_image/%s/deny' % (settings.SERVER_URL , machine_request_id)
...
class MachineRequestAction(APIView):
    ...
    def get(self, request, machine_request_id, action):
        ...
        """
        try:
            mach_request = CoreMachineRequest.objects.get(id=machine_request_id)
        except CoreMachineRequest.DoesNotExist as no_obj:
        ...
        #Don't over-write a request thats already been approve/complete/error/deny
...
class MachineRequest(APIView):
    ...
    def patch(self, request, provider_id, identity_id, machine_request_id):
        ...
        serializer = MachineRequestSerializer(mach_request, data=data, partial=True)
        if serializer.is_valid():
            serializer.save()
