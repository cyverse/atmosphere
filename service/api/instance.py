...
from core.models.tag import updateTags
...
class InstanceList(APIView):
    ...
    def get(self, request, provider_id, identity_id):
        ...
        core_instance_list = [convertEshInstance(inst, provider_id, user) for inst in esh_instance_list]
        #TODO: Filter out 'instances you shouldn't see' -- Core/Auth links with esh..
    ...
    def post(self, request, provider_id, identity_id, format=None):
        ...
        TODO: Create a 'reverse' using the instance-id to pass the URL for the newly created instance
        ...
        coreInstance = convertEshInstance(eshInstance, provider_id, user, token)
...
class InstanceAction(APIView):
    ...
    def post(self, request, provider_id, identity_id, instance_id):
        ...
        if not action_params.get('action',None):
            errorObj = failureJSON([{'code':400, 'message':'POST request to /action require a BODY with \'action\':'}])
            return Response(errorObj, status=status.HTTP_400_BAD_REQUEST)
        esh_driver = prepareDriver(request, identity_id)
        logger.warn(esh_driver)
        ...
        try:
            ...
            if 'volume' in action:
                ...
                if 'attach_volume' in action:
                    result = esh_driver.attach_volume(eshInstance, eshVolume, device)
                elif 'detach_volume' in action:
                    result= esh_driver.detach_volume(eshVolume)
                ...
                while True:
                    ...
                    if attempts >= 6: #After 6 attempts (~1min)
                        break
                    if 'attaching' not in eshVolume.extra['status'] and 'detaching' not in eshVolume.extra['status']:
                        break
                    time.sleep(2**attempts)#Exponential backoff..
                    attempts  += 1
                logger.debug("%s completed in %s attempts" % (action, attempts))
            ...
            logger.warn(ne)
            errorObj = failureJSON([{'code':404, 'message':'The requested action %s is not available on this provider' % action_params['action']}])
...
class Instance(APIView):
    ...
    def get(self, request, provider_id, identity_id, instance_id):
        ...
        """
        logger.warn('GET found -- %s' % instance_id)
        user = request.user
        esh_driver = prepareDriver(request, identity_id)
        ...
        try:
            ...
            errorObj = failureJSON([{'code':404,'message':'Instance does not exist'}])
            return Response(errorObj, status=status.HTTP_404_NOT_FOUND)
    ...
    def patch(self, request, provider_id, identity_id, instance_id):
        ...
        try:
            ...
            logger.warn('Authentication Failed. Provider-id:%s Identity-id:%s' % (provider_id,identity_id))
            errorObj = failureJSON([{'code':400,'message':'Bad Provider/Identity'}])
            return Response(errorObj, status=status.HTTP_400_BAD_REQUEST)
    ...
    def delete(self, request, provider_id, identity_id, instance_id):
        ...
        try:
            eshInstance = esh_driver.get_instance(instance_id)
            result = esh_driver.destroy_instance(eshInstance)
            eshInstance = esh_driver.get_instance(instance_id)
            coreInstance = convertEshInstance(eshInstance, provider_id, user)
            if coreInstance:
