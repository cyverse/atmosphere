...
class IdentitySerializer(serializers.ModelSerializer):
    ...
    #URLs
    #instances = serializers.HyperlinkedIdentityField(view_name='instance-list', format='html')
    #volumes = serializers.HyperlinkedIdentityField(view_name='volume-list', format='html')
    #machines = serializers.HyperlinkedIdentityField(view_name='machine-list', format='html')
    class Meta:
        model = Identity
...
class InstanceSerializer(serializers.ModelSerializer):
    ...
    machine_name = serializers.CharField(read_only=True, source='esh_machine_name')
    machine_alias_hash = serializers.CharField(read_only=True, source='hash_machine_alias')
    ip_address = serializers.CharField(read_only=True)
    start_date = serializers.DateTimeField(read_only=True)
    token = serializers.CharField(read_only=True)
    #Writeable fields
    name = serializers.CharField(source='name')
    tags = serializers.ManySlugRelatedField(slug_field='name', source='tags', read_only=False)
...
class MachineRequestSerializer(serializers.ModelSerializer):
    """
    Original Request:
    name=Test&instance_id=i-3F57078F&ip_address=128.196.142.6&provider=2&description=Test_Desc&installed_software=Test_sw&sys=Test_Sys&exclude=Test_file&tags=debug&vis=private
    Fails:
    ...
    provider = serializers.PrimaryKeyRelatedField(source='new_machine_provider')
    owner = serializers.SlugRelatedField(slug_field='username', source='new_machine_owner')
    ...
    class Meta:
        ...
        fields = ('id', 'instance', 'status', 'name', 'owner', 'provider', 'vis', 'description', 'tags', 'sys', 'software', 'shared_with')
        #exclude = ('new_machine', 'parent_machine', )
...
class ProviderMachineSerializer(serializers.ModelSerializer):
    ...
    alias_hash = serializers.CharField(read_only=True, source='hash_alias')
    created_by = serializers.CharField(read_only=True, source='machine.created_by.username')
    ...
    architecture = serializers.CharField(read_only=True, source='esh_architecture')
...
class GroupSerializer(serializers.ModelSerializer):
    ...
    def get_identities(self, group):
        ...
        return map(lambda i: {"id": i.id, "provider_id": i.provider_id}, identities)
