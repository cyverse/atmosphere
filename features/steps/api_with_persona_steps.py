...
def set_key_to_persona_var_to_allocation(context, key, allocation_source_name):
    assert context.persona is not None, u'no persona is setup'
    import core.models
    allocation_source = core.models.AllocationSource.objects.get(
...
def get_details_for_active_instance(context):
    ...
    url = django.urls.reverse(
        'api:v2:instance-detail', args=(active_instance.provider_alias, )
    )
...
@when(
    'I assign allocation source "{allocation_source_name}" to active instance'
)
def assign_allocation_source_to_active_instance(
    context, allocation_source_name
):
    assert context.persona
    active_instance = context.persona['active_instance']
    client = context.persona['client']
...
def change_name_of_active_instance(context, new_instance_name):
    ...
    response = client.patch(
        '/api/v2/instances/{}'.format(active_instance.id),
...
def api_response_code_is(context, response_code):
    ...
    context.test.assertEqual(
        context.persona['response'].status_code, response_code
    )
...
def api_response_contains(context):
    ...
    if isinstance(expected_response, collections.Sequence
                 ) and hasattr(expected_response, '__iter__'):
        context.test.assertSequenceRecursive(expected_response, response.data)
    else:
        context.test.assertDictContainsSubsetRecursive(
...
def assertSequenceRecursive(self, expected, actual, depth=0, msg=None):
    ...
    self.assertEqual(
        len(expected),
        len(actual),
        msg='Lengths are different: {} vs {}'.format(expected, actual)
    )
    for expected_item, actual_item in zip(expected, actual):
        if isinstance(actual_item, datetime.datetime):
            actual_item = datetime.datetime.strftime(
                actual_item, '%Y-%m-%d %H:%M:%S%z'
            )
        if isinstance(expected_item, datetime.datetime):
            expected_item = datetime.datetime.strftime(
                expected_item, '%Y-%m-%d %H:%M:%S%z'
            )
        ...
        elif isinstance(actual_item, collections.Sequence
                       ) and hasattr(actual_item, '__iter__'):
...
def assertDictContainsSubsetRecursive(
    self, expected, actual, depth=0, msg=None
):
    assert isinstance(self, TestCase)
    ...
    for key, value in expected.iteritems():
        ...
        if isinstance(actual[key], datetime.datetime):
            actual[key] = datetime.datetime.strftime(
                actual[key], '%Y-%m-%d %H:%M:%S%z'
            )
        if key not in actual:
            ...
            missing.extend(missing_1)
            mismatched.extend(mismatched_1)
        elif isinstance(actual[key], collections.Sequence
                       ) and hasattr(actual[key], '__iter__'):
            missing_1, mismatched_1 = self.assertSequenceRecursive(
                value, actual[key], depth=depth + 1
            )
            missing.extend(missing_1)
...
@when(
    'we increase compute used by {compute_used:d} for the user on "{allocation_source_name}"'
)
def increase_compute_used_for_user_on_allocation_source(
    context, compute_used, allocation_source_name
):
    assert context.persona
    context.test.assertIn('user', context.persona)
    user = context.persona['user']
...
def my_time_remaining_on_allocation_source(
    context, allocation_source_name, time_remaining
):
    assert context.persona
    context.test.assertIn('user', context.persona)
    user = context.persona['user']
