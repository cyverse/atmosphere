...
def run_monitor_allocation_source(context):
    ...
    never_enforce = [
        setting['allocation_source'] for setting in override_settings
        if setting['override'] == 'NEVER_ENFORCE'
    ]
    always_enforce = [
        setting['allocation_source'] for setting in override_settings
        if setting['override'] == 'ALWAYS_ENFORCE'
    ]
    from service.tasks.monitoring import monitor_allocation_sources
    with mock.patch(
        'service.tasks.monitoring.allocation_source_overage_enforcement_for_user',
        autospec=True
    ) as allocation_source_overage_enforcement_for_user:
...
def allocation_source_overage_enforcement_for_user_called(context):
    context.test.assertTrue(
        hasattr(context, 'allocation_source_overage_enforcement_for_user')
    )
...
def step_impl(context, username, allocation_source_name, yes_or_no):
    ...
    try:
        ...
        with mock.patch(
            'service.instance.settings', autospec=True
        ) as mock_settings:
...
def change_renewal_strategy_command_fired(context, new_renewal_strategy):
    ...
    context.response = context.client.patch(
        '/api/v2/allocation_sources/%s' % context.source_id,
        json.dumps({
...
def renewal_strategy_changed(context, renewal_strategy_is_changed):
    ...
    if not result:
        ...
        assert (
            result == _str2bool(renewal_strategy_is_changed.lower())
            and context.response.data['renewal_strategy'] == context.
...
def name_is_changed_is(context, name_is_changed):
    ...
    if not result:
        ...
        assert result == _str2bool(
            name_is_changed.lower()
        ) and context.response.data['name'] == context.new_name
...
def change_compute_allowed_command_fired(context, new_compute_allowed):
    context.response = context.client.patch(
        '/api/v2/allocation_sources/%s' % context.source_id,
        json.dumps({
            "compute_allowed": int(new_compute_allowed)
        }),
        content_type='application/json'
    )
...
def compute_allowed_changed(context, compute_allowed_is_changed):
    ...
    if not result:
        ...
        assert (
            result == _str2bool(compute_allowed_is_changed.lower()) and context.
            response.data['compute_allowed'] == context.new_compute_allowed
        )
...
@when(
    'the user allocation snapshot for "{username}" and "{allocation_source_name}" is deleted'
)
def delete_user_allocation_snapshot(context, username, allocation_source_name):
    ...
    context.test.assertIsInstance(
        user_allocation_snapshot, core.models.UserAllocationSnapshot
    )
...
@step(
    'time remaining on allocation source "{allocation_source_name}" is {time_remaining:f}'
)
def time_remaining_on_allocation_source(
    ...
    context.test.assertAlmostEqual(
        actual_remaining_compute, Decimal(time_remaining), delta=0.1
    )
