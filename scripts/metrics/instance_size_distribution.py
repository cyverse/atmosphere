...
def main():
    ...
    try:
        ...
        date_value = now()
    size_distribution = instance_size_distribution(provider.id, args.days, date_value)
    print size_distribution

def _instantiate_size_distribution(provider_id):
    size_distribution = OrderedDict()
    for size in Size.objects.filter(provider__id=provider_id).order_by('cpu','mem','id'):
...
def instance_size_distribution(provider_id, days_ago, now_time, is_now=True):
    ...
    if is_now:
        instances = Instance.objects.filter(Q(end_date__gt=DATE) | Q(end_date__isnull=True), source__provider__id=provider_id)
    else:
        instances = Instance.objects.filter(Q(end_date__gt=DATE), source__provider__id=provider_id)
    for instance in instances:
        unique_sizes = instance.instancestatushistory_set.values_list('size', flat=True).distinct()
