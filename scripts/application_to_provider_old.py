...
def main():
    ...
    # Todo is application.created_by.username the right thing to look at?
    ...
    # Todo may also/instead need to store app_member_uuids if we are setting metadata outside the ImageManager.upload_image function
    app_member_names = []
    ...
    for app_version in application.all_versions:
        ...
        for provider_machine in provider_machines:
            if provider_machine.provider == dest_provider:
                dest_provider_machine = provider_machine
                # todo we don't need to specify a destination provider
