...
from iplantauth.protocol.ldap import lookupUser
import subprocess
import sys
import datetime
import os
...
def _build_node_maps():
    ...
    boto_instances = driver.get_list(
        "DescribeNodes", {},
        [('euca:item', BotoInstance)],
        '/')
    last_node = ''
    nodes = {}
...
def instance_report():
    ...
    for (node, instance_list) in node_map.items():
        args_list = [(node, instances, instance_id)
...
def email_report(*args, **kwargs):
    ...
    send_mail(
        'no-reply@iplantcollaborative.org',
        email_addr,
        'Instance Report',
        'Report generated on %s' %
        now.strftime("%Y-%m-%d at %I:%M:%S %p"),
        files=['/tmp/instance_report.xls'])
