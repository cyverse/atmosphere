...
class InstanceAccessTests(APITestCase):
    ...
    def test_create_and_attempt_delete_by_share_user(self):
        ...
        self.assertEquals(
            ...
            % (response.status_code, response.data))
        try:
            error_message = str(response.data['errors'][0]['message'])
            expected_error = "Only user %s can delete the request for Instance Access" % self.user.username
            self.assertEquals(
            ...
            self.fail(
                    "Request data does not match expected format "
                    "(['errors'][0]['message']): %s" % response.data)
        ...
        self.assertEqual(
            "pending", status_name,
            "Unexpected status after update: %s != 'pending'" % status_name
        )
