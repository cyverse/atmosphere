...
from api.v2.views import InstanceActionViewSet as ViewSet
from .factories import UserFactory, AnonymousUserFactory, GroupFactory, InstanceActionFactory
...
class GetListTests(APITestCase):
    def setUp(self):
        ...
        self.instance_action = InstanceActionFactory.create()

        factory = APIRequestFactory()
        url = reverse('api_v2:instanceaction-list')
...
class GetDetailTests(APITestCase):
    def setUp(self):
        ...
        factory = APIRequestFactory()
        url = reverse('api_v2:instanceaction-detail', args=(self.instance_action.id,))
        self.request = factory.get(url)
        force_authenticate(self.request, user=self.user)
        self.response = self.view(self.request, pk=self.instance_action.id)

    def test_is_not_public(self):
        force_authenticate(self.request, user=self.anonymous_user)
        response = self.view(self.request, pk=self.instance_action.id)
        self.assertEquals(response.status_code, 403)
