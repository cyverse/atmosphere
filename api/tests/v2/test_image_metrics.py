...
class ImageMetricsTest(APITestCase, APISanityTestCase):
    ...
    def setUp(self):
        ...
        self.staff_user = UserFactory.create(
            username='test-staffuser', is_staff=True, is_superuser=True
        )
    ...
    def test_user_sees_no_statistics(self):
        ...
        url = reverse(
            self.url_route + '-detail', args=(self.application.uuid, )
        )
        response = client.get(url)
        self.assertEquals(response.status_code, 404)

    @skip(
        "Test is non deterministic and yields different results based on its redis cache"
    )
    ...
    @skip(
        "Skipping until we know how we want to measure statistics like these."
    )
