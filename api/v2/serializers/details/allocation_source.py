...
class AllocationSourceSerializer(serializers.HyperlinkedModelSerializer):
    ...
    def _get_user_allocation_snapshot(self, allocation_source, attr_name):
        ...
        snapshot = UserAllocationSnapshot.objects.filter(
            allocation_source=allocation_source, user=user
        ).first()
        if not snapshot:
            return None
        attr = getattr(snapshot, attr_name)
    ...
    def get_global_burn_rate(self, allocation_source):
        ...
        if allocation_source.name in special_allocation_sources:
            ...
            return self._get_allocation_source_snapshot(
                allocation_source, 'global_burn_rate'
            )
    ...
    def get_user_burn_rate(self, allocation_source):
        return self._get_user_allocation_snapshot(
            allocation_source, 'burn_rate'
        )

    def get_user_compute_used(self, allocation_source):
        return self._get_user_allocation_snapshot(
            allocation_source, 'compute_used'
        ) or Decimal(0.000)
    ...
    def get_compute_allowed(self, allocation_source):
        ...
        if allocation_source.name in special_allocation_sources:
            sub_allocation_compute_allowed = special_allocation_sources[
                allocation_source.name]['compute_allowed']
    ...
    class Meta:
        ...
        fields = (
            ...
            'end_date', 'compute_used', 'global_burn_rate', 'updated', 'uuid',
            'renewal_strategy', 'user_compute_used', 'user_burn_rate',
            'user_snapshot_updated'
        )
