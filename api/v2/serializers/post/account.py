...
class AccountSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        validated_data['atmo_user'], validated_data[
            'atmo_group'] = self._validate_user_group(data)
    ...
    def _validate_user_group(self, data):
        ...
        if atmo_user:
            atmo_group = Group.objects.filter(user=atmo_user
                                             ).filter(name=groupname).first()
        else:
            atmo_group = None
        if not atmo_user and not create_account:
            raise serializers.ValidationError(
                "User %s does not exist, and 'create_account' is False." %
                username
            )
        if not atmo_group and not create_account:
            raise serializers.ValidationError(
                "Group %s does not exist, and 'create_account' is False." %
                groupname
            )
    ...
    def _validate_provider(self, provider_uuid):
        ...
        prov_qs = provider_manager.filter(only_current(), active=True)
        provider = prov_qs.filter(uuid=provider_uuid).first()
        if not provider:
            raise serializers.ValidationError(
                "Cannot create an account for provider with UUID %s" %
                provider_uuid
            )
        return provider
    ...
    def _validate_openstack_credentials(self, credentials, required_keys):
        ...
        if missing_keys:
            raise serializers.ValidationError(
                "Missing required key(s) for Openstack creation: %s" %
                missing_keys
            )
        return credentials
    ...
    def _validate_credentials(self, provider, credentials, required_keys):
        ...
        if provider_type == 'openstack':
            valid_creds = self._validate_openstack_credentials(
                credentials, required_keys
            )
        else:
    ...
    def validate_user(self, provider_uuid):
        ...
        if request_user.admin_providers.filter(uuid=provider_uuid).count() > 0:
            return
        raise serializers.ValidationError(
            "Only the Cloud Administrators can create new accounts"
        )
    ...
    def _create_openstack_account(self, provider, credentials):
        ...
        try:
            ...
            raise serializers.ValidationError(
                "The credentials used to create this account are invalid. Double-check your credentials and try again."
            )
        except:
    ...
    def create_openstack_identity(self, user, provider, validated_data):
        ...
        if create_account and has_account_provider:
            ...
            ) = self._create_openstack_account(provider, credentials_dict)
        ...
        identity = self._get_or_create_identity(
            user, provider, validated_data['quota']
        )
        for key, value in credentials_dict.items():
            identity.credential_set.get_or_create(key=key, value=value)
    ...
    class Meta:
        fields = (
            'atmo_user', 'atmo_group', 'admin_account', 'create_account',
            'credentials', 'provider', 'quota'
        )
