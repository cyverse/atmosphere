...
class WebTokenView(RetrieveAPIView):
    ...
    def retrieve(self, request, pk=None):
        ...
        try:
            ...
            return HttpResponse(
                "Atmosphere failed to retrieve web token", status=500
            )
    ...
    def guacamole_token(self, ip_address):
        ...
        guacamole_color = UserProfile.objects.get(
        ...
        logger.info(
            "User %s initiated %s connection to %s" %
            (atmo_username, protocol.upper(), ip_address)
        )
        ...
        signature = hmac.new(guac_secret, message, hashlib.sha256
                            ).digest().encode("base64").rstrip('\n')

        # Build the POST request
        request_string = (
            'timestamp=' + timestamp + '&guac.port=' + port + '&guac.username='
            + atmo_username + '&guac.password=' + passwd + '&guac.protocol=' +
            protocol + '&signature=' + signature + '&guac.hostname=' +
            ip_address + '&id=' + conn_id + '&guac.sftp-username=' +
            atmo_username + '&guac.sftp-directory=/home/' + atmo_username +
            '&guac.enable-sftp=true'
        )

        if protocol == "ssh":
            request_string += "&guac.color-scheme=" + guacamole_color.replace(
                "_", "-"
            )

        # Send request to Guacamole backend and record the result
        response = requests.post(
            guac_server + '/api/tokens', data=request_string
        )
