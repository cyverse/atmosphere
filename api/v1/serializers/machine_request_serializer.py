...
class MachineRequestSerializer(serializers.ModelSerializer):
    ...
    sys = serializers.CharField(
        default="", source='system_files', allow_blank=True, required=False
    )
    software = serializers.CharField(
        default="No software listed",
    ...
    name = serializers.CharField(
        source='new_application_name', validators=[no_special_characters]
    )
    ...
    version_name = serializers.CharField(
        source='new_version_name', default="1.0", required=False
    )
    version_changes = serializers.CharField(
        source='new_version_change_log',
        default="1.0 - New Version Created",
        required=False
    )
    fork = serializers.BooleanField(source='new_version_forked', required=False)
    description = serializers.CharField(
        source='new_application_description', allow_blank=True, required=False
    )
    tags = serializers.CharField(
        source='new_version_tags', required=False, allow_blank=True
    )
    threshold = NewThresholdField(
        source='new_version_threshold', required=False
    )
    ...
    class Meta:
        ...
        fields = (
            'id', 'instance', 'status', 'request_status', 'name', 'owner',
            'provider', 'vis', 'description', 'tags', 'sys', 'software',
