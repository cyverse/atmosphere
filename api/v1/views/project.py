...
class ProjectInstanceExchange(APIView):
    ...
    def put(self, request, project_uuid, instance_id):
        ...
        if not instance:
            return Response(
                "instance with ID=%s not found in the database" % (instance_id),
                status=status.HTTP_400_BAD_REQUEST
            )
        instance = instance[0]
    ...
    def delete(self, request, project_uuid, instance_id):
        ...
        if not project:
            return Response(
                "Project with ID=%s does not exist" % project_uuid,
                status=status.HTTP_400_BAD_REQUEST
            )
...
class ProjectVolumeExchange(APIView):
    ...
    def put(self, request, project_uuid, volume_id):
        ...
        if not volume:
            return Response(
                "volume with ID=%s not found in the database" % (volume_id, ),
                status=status.HTTP_400_BAD_REQUEST
            )
        volume = volume[0]
...
class ProjectDetail(AuthAPIView):
    def patch(self, request, project_uuid):
        ...
        serializer = ProjectSerializer(
            project, data=data, partial=True, context={"request": request}
        )
        if serializer.is_valid():
            serializer.save()
            response = Response(serializer.data, status=status.HTTP_201_CREATED)
