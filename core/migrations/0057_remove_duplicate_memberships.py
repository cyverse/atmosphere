...
class Migration(DataMigration):
    ...
    def drop_provider_membership_dupes(self, membership_list):
        provider_seen = set()
        add_provider = provider_seen.add
        duplicate_membership  = set( m for m in membership_list
                        if m.provider in provider_seen or add_provider(m.provider))
        provider_seen = list(provider_seen)
        duplicate_membership = list(duplicate_membership)
        if duplicate_membership:
            for membership in duplicate_membership:
                membership.delete()

    def drop_instance_membership_dupes(self, membership_list):
        instance_seen = set()
        add_instance = instance_seen.add
        duplicate_membership  = set( m for m in membership_list
                        if m.instance in instance_seen or add_instance(m.instance))
        instance_seen = list(instance_seen)
        duplicate_membership = list(duplicate_membership)
        if duplicate_membership:
            for membership in duplicate_membership:
                membership.delete()

    def drop_identity_membership_dupes(self, membership_list):
        identity_seen = set()
        add_identity = identity_seen.add
        duplicate_membership  = set( m for m in membership_list
                        if m.identity in identity_seen or add_identity(m.identity))
        identity_seen = list(identity_seen)
        duplicate_membership = list(duplicate_membership)
        if duplicate_membership:
            for membership in duplicate_membership:
                membership.delete()
    ...
    def forwards(self, orm):
        ...
        for group in sorted(orm.Group.objects.all()):
            self.drop_provider_membership_dupes(group.providermembership_set.all())
            self.drop_instance_membership_dupes(group.instancemembership_set.all())
            self.drop_identity_membership_dupes(group.identitymembership_set.all())
    ...
    models = {
        ...
        'core.flow': {
            ...
            'start_date': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2014, 6, 18, 0, 0)'}),
