...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """
        Whew.. That was a lot of typing for a migration no one will
        ever read!
        """
        ...
        ish_list = orm.InstanceStatusHistory.objects.all()
        if not ish_list:
            #Nothing to see here, migration completed.
            return
        print ">> NOTE: There are %d InstanceStatusHistory objects with no associated Size." % len(ish_list)
        print ">> If the instances associated with these objects still exist,"
        print ">> the LAST history for that object will be updated with the current size"
        print ">> In all other cases:"
        print ">> The size for the History objects cannot be reliably determined"
        print ">> Therefore an 'UnknownSize' object has been created as a placeholder."
        print ">> The placeholder values can be adjusted in the database,"
        print ">> but have NO representation on their provider."
        print ">> Adding 'Unknown Size'.."
        

        provider_placeholder = {}
        provider_counter = {}
        distinct_providers = set([ish.instance.provider_machine.provider for ish in ish_list])
        for provider in distinct_providers:
            #NOTE: Using 1 here so that Multipliers are not adversely affected.
            placeholder, created = orm.Size.objects.get_or_create(
                    name="Unknown Size", alias="N/A", provider=provider,
                    cpu=1, disk=1, root=1, mem=1)
            if created:
                #End date this placeholder
                placeholder.end_date = timezone.now()
                placeholder.save()
            provider_placeholder[provider] = placeholder

        for key in provider_placeholder.keys():
            provider_counter[key] = 0
        for ish in ish_list:
            provider = ish.instance.provider_machine.provider
            ish.size = provider_placeholder[provider]
            current_count = provider_counter[provider]
            provider_counter[provider] = current_count + 1
            ish.save()
        print ">> Total Unknown Size added:"
        for (provider,count) in provider_counter.items():
            print ">> Provider:%s Count:%d" % (provider.location,count)
    ...
    models = {
        ...
        'core.flow': {
            ...
            'start_date': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2014, 4, 14, 0, 0)'}),
