    ...
    with open(filename, 'w') as the_file:
        ...
        for mr in query.order_by('start_date', 'end_date'):
            the_file.write("%s,%s,%s,%s,%s,%s,%s,%s\n" % (
                mr.id, mr.created_by.username, mr.new_machine_provider.location,
                mr.new_machine.application.name if mr.new_machine else "N/A",
                mr.new_machine.application_version.name
                if mr.new_machine else "N/A", mr.start_date.strftime("%x %X"),
                mr.end_date.strftime("%x %X")
                if mr.end_date else "N/A", "public"
