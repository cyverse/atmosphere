...
def instance_history_usage_report(
    filename, start_date=None, end_date=None, only_active=False
):
    query = InstanceStatusHistory.objects.all()
    if only_active:
        query = query.filter(
            Q(status__name='active') | Q(status__name='running')
        )
    if start_date:
    ...
    with open(filename, 'w') as the_file:
        the_file.write(
            "ID,Provider Alias,Username,Provider,Application,Version,Machine UUID,Status,Start Date,End Date,Active Time(Hours),CPU,RAM,DISK\n"
        )
        for instance_history in query.order_by(
            'instance__id', 'start_date', 'end_date'
        ):
...
def instance_usage_report(filename, start_date=None, end_date=None):
    ...
    with open(filename, 'w') as the_file:
        the_file.write(
            "ID,Provider Alias,Username,Provider,Application,Version,Machine UUID,Start Date,End Date,Active Time(Hours),CPU,RAM,DISK\n"
        )
...
def machine_request_report(filename, start_date=None, end_date=None):
    ...
    with open(filename, 'w') as the_file:
        the_file.write(
            "ID,Username,Provider,Application,Version,Start Date,Created on,Visibility\n"
        )
        for mr in query.order_by('start_date', 'end_date'):
            the_file.write(
                "%s,%s,%s,%s,%s,%s,%s,%s\n" % (
                    mr.id, mr.created_by.username,
                    mr.new_machine_provider.location,
                    mr.new_machine.application.name if mr.new_machine else
                    "N/A", mr.new_machine.application_version.name if
                    mr.new_machine else "N/A", mr.start_date.strftime("%x %X"),
                    mr.end_date.strftime("%x %X") if mr.end_date else "N/A",
                    "public"
                    if mr.new_application_visibility == "public" else "private"
                )
            )
...
def monthly_metrics(filename, start_date, end_date):
    monthly_breakdown = list(
        rrule.rrule(
            dtstart=start_date, freq=rrule.MONTHLY, until=timezone.now()
        )
    )
    with open(filename, 'w') as the_file:
        the_file.write(
            "Start Date,End Date,Instances launched,Users Joined,Applications Created,Machines Added\n"
        )
        for idx, month in enumerate(monthly_breakdown):
            ...
            provider_ids = Provider.objects.filter(
                active=True, end_date__isnull=True
            ).values_list(
                "id", flat=True
            )
            the_file.write(
                _print_csv_row_between_dates(
...
def _print_csv_row_between_dates(
    provider_ids, start_date, end_date, pretty_print=False
):
    instances = Instance.objects.filter(
        start_date__gt=start_date, start_date__lt=end_date
    )
    users = AtmosphereUser.objects.filter(
        date_joined__gt=start_date, date_joined__lt=end_date
    )
    ...
    if pretty_print:
        return """Between %s and %s:
            ...
            start_date, end_date, instances.count(), users.count(),
            apps.count(), machines.count()
        )
    return "%s,%s,%s,%s,%s,%s" % (
