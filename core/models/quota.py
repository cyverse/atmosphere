...
class Quota(models.Model):
    """
    Quota limits the amount of resources that can be used for a User/Group and the Identity they are in charge of (see IdentityMembership - group.py)
    """
    ...
    storage_count = models.IntegerField(null=True, blank=True, default=2) #In # Volumes allowed
    def __unicode__(self):
        return "CPU:%s, MEM:%s, DISK:%s DISK #:%s" % (self.cpu,self.memory,self.storage, self.storage_count)
    @classmethod
    def defaults(self):
        return {
