...
class Machine(models.Model):
    """
    Machines are described with their name, tags, and a lengthy description of what is included in the machine.
    A machine has an icon/logo for use in frontend applications
    On launch, new instances will request an init_package to run additional runtime configuration scripts.
    Private machines can be 'shared' with other groups using MachineMembership (see group.py)
    Machines creation and deletion date, as well as the user who created the machine, are recorded for logging purposes.
    ...
    created_by = models.ForeignKey(User) #The specific user that requested imaging OR ADMIN/SU
...
class ProviderMachine(models.Model):
    """
    Machines are created by Providers, and multiple providers can implement a single machine (I.e. Ubuntu 12.04)
    However each provider will have a specific, unique identifier to represent that machine. (emi-12341234 vs ami-43214321)
    """
    esh = None #Field is ONLY Filled after converting an eshMachine
    ...
    def find_machine_owner(self):
        if provider.location == 'EUCALYPTUS':
    ...
    def __unicode__(self):
        return "%s (Provider:%s - Machine:%s) " % (self.identifier, self.provider, self.machine)
    class Meta:
...
def build_cached_machines():
    ...
    logger.debug("built core machines dictionary with %s machines." % len(ProviderMachine.cached_machines))
...
def findProviderMachine(provider_alias, provider_id):
    ...
    return ProviderMachine.cached_machines.get((int(provider_id), provider_alias))

def loadMachine(machine_name, provider_alias, provider_id):
    """
    Returns List<ProviderMachine> - Each object contains reference to a new machine-alias combination
...
def createProviderMachine(machine_name, provider_alias, provider_id, description=None):
    ...
    provider_machine = ProviderMachine.objects.create(machine=machine, provider=provider, identifier=provider_alias) 
    logger.info("New ProviderMachine created: %s" % provider_machine)
    if ProviderMachine.cached_machines:
        ProviderMachine.cached_machines[(provider_machine.provider.id, provider_machine.identifier)] = provider_machine
...
def getGenericMachine(name):
    try:
        ...
        return machine
    except Machine.DoesNotExist, dne:
        return None
    except Machine.MultipleObjectsReturned, mor:
...
def createGenericMachine(name, description, creator=None):
    ...
    new_mach = Machine.objects.create(name=name, description=description, created_by=creator)
    return new_mach
