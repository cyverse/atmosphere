...
def email_admin(request, subject, message):
    ...
    body %= (message,
             location,
             user, remote_ip, #Sent From:
             user_agent, resolution) #Sent By:
...
"""  % (instance_id, ip, linuxusername, datetime.now().strftime('%b, %d %Y %H:%M:%S'))
...
Thank you for using atmosphere! If you have any questions please contact: support@iplantcollaborative.org""" % (user.username, new_machine.identifier, name)
