...
def cas_logoutRedirect():
    return HttpResponseRedirect(settings.CAS_SERVER+"/cas/logout?service="+settings.SERVER_URL)
...
def createAuthToken(username):
    """
    #TODO: Remove remote_ip from auth.models.Token unless theres a reason to keep it..
    returns a new token for username
    """
    user = User.objects.get(username=username)
    auth_user_token = AuthToken(user=user, api_server_url=settings.API_SERVER_URL)
