Atmo.Models.Volume = Atmo.Models.Base.extend(
  {
    ...
    parse: function(response) {
      ...
      attributes.create_relative = Atmo.Utils.relative_time(new Date(response.start_date));
      var attach_data = response.attach_data;
      if(!jQuery.isEmptyObject(attach_data)) {
        attributes.attach_data_attach_time =
          new Date(attach_data.attachTime);
        attributes.attach_data_attach_relative =
          Atmo.Utils.relative_time(attributes.attach_data_attach_time);
    ...
    attach_to: function(instance, options) {
      ...
      if (!options.error) options.error = function() {
          ...
          var body = "You can refresh the page and try to perform this operation again. If the problem persists, please email <a href=\"mailto:support@iplantcollaborative.org\">supprt@iplantcollaborative.org</a>. <br /><br />We apologize for the inconvenience.";
      ...
      var self = this,
          action_url = instance.url()+"action/";
      $.ajax({
          url:action_url, 
          type:"POST", 
          data:param, 
