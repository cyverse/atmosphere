...
define(['backbone', 'templates', 'jquery-ui', 'utils'], function(Backbone, Templates, jqui, Utils) {
    ...
    var VolumeScreenInstance = Backbone.View.extend({
        tagName: 'li',
        ...
        on_drop: function(event, ui) {
            ...
            if (volume.get('status') == 'in-use') 
                Utils.notify("ERROR", "Volume is not ready. Please refresh and try again.");
            else {
                Utils.attach_volume(volume, this.model, null);
                $(ui.draggable).remove();
            };
        }
    });

    return VolumeScreenInstance;
