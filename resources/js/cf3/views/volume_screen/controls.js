...
var VolumeScreenControls = Backbone.View.extend({
    'tagName': 'div',
    'className' : 'row',
    template: _.template(Templates.volume_screen_controls),
    events: {
    ...
    initialize: function(options) {
        ...
        this.instances.bind("add", this.render_attach_detach, this);
        this.instances.bind("remove", this.render_attach_detach, this);
        this.instances.bind("change:state", this.render_attach_detach, this);
        this.volumes.bind("sync", this.render_attach_detach, this);
        this.volumes.bind("add", this.render_attach_detach, this);
        this.volumes.bind("remove", this.render_attach_detach, this);
        this.volumes.bind("change", this.render_attach_detach, this);
    },
    render: function() {
    ...
    render_attach_detach: function() {
        ...
        for (var i = 0; i < this.instances.models.length; i++) {
            ...
            else if (this.instances.models[i].get('state_is_build')) {
                pending_instances++;
            }
        }
        ...
        if (this.volumes.models.length > 0 && available_instances > 0) {
        ...
        if (available_instances == 0 && this.volumes.models.length > 0) {
            ...
            self.$el.find('select[name="all_volumes"]').after($('<span>', {
                html: function() {
                    ...
                    if (this.instances.models.length == 0) {
